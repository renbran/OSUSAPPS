...
//    Extending Component
    setup(){
        ...
        onWillStart(this.loadSaleTypes);
        onWillStart(this.loadDashboardData);
    }
    async loadSaleTypes() {
        // Fetch sale types from le_sale_type
        const sale_types = await this.orm.searchRead("le.sale.type", [], ["id", "name"]);
        this.state.sale_types = sale_types;
    }
    async loadDashboardData() {
        try {
            const params = {
                ...
                sale_type: this.state.sale_type_id,
            };
            const dashboard = await this.orm.call("sale.order", "get_dashboard_data", [params.date_from, params.date_to, params.sale_type]);
    ...
    renderSaleTypeChart() {
        if (!this.state.dashboard.sale_type_counts) return;
        const ctx = document.getElementById("salesTypeChart");
        if (!ctx) return;
        new Chart(ctx, {
            type: "pie",
            data: {
                labels: Object.keys(this.state.dashboard.sale_type_counts),
                datasets: [{ data: Object.values(this.state.dashboard.sale_type_counts) }],
            },
        });
    }
}
registry.category("actions").add("all_in_one_sales_kit.sales_dashboard", SalesDashboard);
            lead_customer_data: {},
            date:{},
        })
