...
class SalesReport(models.Model):
    ...
    # Enhancement fields from related modules
    booking_date = fields.Date(string="Booking Date")  # osus_invoice_report
    sale_type_id = fields.Many2one('le.sale_type', string="Sale Type")  # le_sale_type
    agent_id = fields.Many2one('res.partner', string="Agent")  # commission_ax
    commission_amount = fields.Float(string="Commission Amount")

    @api.model
    def get_dashboard_data(self, date_from, date_to, sale_type=None):
        domain = [('booking_date', '>=', date_from), ('booking_date', '<=', date_to)]
        if sale_type:
            domain += [('sale_type_id', '=', sale_type)]
        orders = self.env['sale.order'].search(domain)
        total_commission = sum(order.commission_amount for order in orders)
        orders_sorted = sorted(orders, key=lambda o: o.amount_total, reverse=True)
        top_sales = orders_sorted[:10]
        total_sales_value = sum(o.amount_total for o in orders)
        average_sales_value = total_sales_value / len(orders) if orders else 0
        sale_type_counts = {}
        for order in orders:
            key = order.sale_type_id.name if order.sale_type_id else _('Undefined')
            sale_type_counts[key] = sale_type_counts.get(key, 0) + 1
        agent_commissions = {}
        for order in orders:
            if order.agent_id:
                agent_commissions[order.agent_id.name] = agent_commissions.get(order.agent_id.name, 0) + order.commission_amount
        return {
            'orders': [
                {
                    'name': o.name,
                    'amount_total': o.amount_total,
                    'margin': getattr(o, 'margin', 0),
                    'agent_id': {'name': o.agent_id.name if o.agent_id else ''},
                    'commission_amount': o.commission_amount,
                    'sale_type_id': {'name': o.sale_type_id.name if o.sale_type_id else ''},
                } for o in orders
            ],
            'top_sales': [
                {
            ...
            'agent_commissions': list(agent_commissions.items()),
        }
