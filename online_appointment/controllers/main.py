...
class OnlineAppointment(http.Controller):
    ...
    def get_days_with_free_slots(self, option_id, appointee_id, year, month, criteria):
        ...
        if not option_id or not year or not month:
            return {}
    ...
    def free_timeslots(self, appointment_option, appointment_with, appointment_date, form_criteria, **kwargs):
        ...
        try:
            ...
            appointee_id = 0
        try:
            splitted = appointment_date.split('/')
            if len(splitted) == 2:
                appointment_date += '/' + str(datetime.date.today().year)

            if len(splitted) == 3 and len(splitted[2]) < 4:
                date_parsed = datetime.datetime.strptime(appointment_date, "%d/%m/%y").date()
            else:
                date_parsed = datetime.datetime.strptime(appointment_date, "%d/%m/%Y").date()
        except:
            date_parsed = False
        ...
        days_with_free_slots = self.get_days_with_free_slots(option_id,
                                                             appointee_id,
                                                             date_parsed.year if date_parsed else False,
                                                             date_parsed.month if date_parsed else False,
                                                             form_criteria)
        ...
        return {
            ...
            'days_with_free_slots': days_with_free_slots,
            'focus_year': date_parsed.year if date_parsed else False,
            'focus_month': date_parsed.month if date_parsed else False,
            'date_parsed': date_parsed.strftime('%d/%m/%Y') if date_parsed else False
        }
